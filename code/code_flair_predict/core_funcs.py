...
def train_epoch_prediction(
    ...
    img_t1_weight: Tensor = _data[DataKey.T1Weight].to(config.device).unsqueeze(1).unsqueeze(2).unsqueeze(3)
    img_t2_weight: Tensor = _data[DataKey.T2Weight].to(config.device).unsqueeze(1).unsqueeze(2).unsqueeze(3)

    img_cnt_minibatch = img_fl.shape[0]
    ...
    output, label = train_part(
        ...
        img_fl=img_fl,
        img_t1=img_t1,
        img_t2=torch.zeros_like(img_t2),
        meta=meta,
    )

    loss_t1 = torch.mean(loss_func(output, label), dim=(1, 2, 3), keepdim=True)
    torch.mean(loss_clip(loss_t1 * img_t1_weight, epoch)).backward()

    output, label = train_part(
        network=network,
        img_fl=img_fl,
        img_t1=torch.zeros_like(img_t1),
        img_t2=img_t2,
        meta=meta,
    )

    loss_t2 = torch.mean(loss_func(output, label), dim=(1, 2, 3), keepdim=True)
    torch.mean(loss_clip(loss_t2 * img_t2_weight, epoch)).backward()
    ...
    loss_t1t2 = torch.mean(loss_func(output, label), dim=(1, 2, 3), keepdim=True)
    torch.mean(loss_clip(loss_t1t2 * (img_t1_weight + img_t2_weight), epoch)).backward()

    torch.nn.utils.clip_grad_norm_(network.parameters(), 5.0)
    step_optimizers(optim_list=optim_list)

    train_state.add("loss", loss_t1 + loss_t2 + loss_t1t2)
...
def test_part_prediction(
    ...
    loss = loss_func(output, img_fl)
    loss = torch.mean(loss, dim=(1, 2, 3), keepdim=True)
    test_state.add("loss", loss)

    for i in range(batch_cnt):
        output_i = output[i : i + 1, :, :, :]
        img_fl_i = img_fl[i : i + 1, :, :, :]
        mask_fl_i = mask_fl[i : i + 1, :, :, :]

        mask_sum = torch.sum(mask_fl_i, dim=(1, 2, 3), keepdim=True)
        if not torch.any(mask_sum == 0):
            output_i = output_i * img_fl_i[mask_fl_i == 1].mean() / output_i[mask_fl_i == 1].mean()
            test_state.add("mse", calculate_mse(output_i, img_fl_i, mask_fl_i))
