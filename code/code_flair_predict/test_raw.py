...
from components.wholebrain_recon import prediction_wholebrain
from core_funcs import ModelType, get_loss_func, log_summary
from datawrapper.datawrapper_raw import DataKey, LoaderConfig, get_data_wrapper_loader
...
def test_part_prediction(
    ...
    img_fl: Tensor = _data[DataKey.FL].to(config.device)
    mask_fl: Tensor = _data[DataKey.FL_Mask].to(config.device)
    img_t1: Tensor = _data[DataKey.T1].to(config.device)
    img_t2: Tensor = _data[DataKey.T2].to(config.device)
    meta: Tensor = _data[DataKey.Meta].to(config.device)
    name: str = _data[DataKey.Name][0]

    logger.info(f"Testing {name} started...")

    data_range = torch.amax(img_fl.view(img_fl.shape[0], -1), dim=1)

    validate_tensors([img_fl, img_t1, img_t2, mask_fl, meta])
    validate_tensor_dimensions([img_fl, img_t1, img_t2, mask_fl], 4)

    batch_cnt = img_fl.shape[0]
    if batch_cnt != 1:
    ...
    for slice in range(output.shape[1]):
        mask_fl_slice = mask_fl[:, slice : slice + 1, :, :]
        output_slice = output[:, slice : slice + 1, :, :]
        img_fl_slice = img_fl[:, slice : slice + 1, :, :]
        output_slice = output[:, slice : slice + 1, :, :]

        loss = loss_func(output_slice, img_fl_slice)
        loss = torch.mean(loss, dim=(1, 2, 3), keepdim=True)
        test_state.add("loss", loss)

        if torch.any(mask_fl_slice):
            ssim = calculate_ssim(output_slice, img_fl_slice, mask_fl_slice, data_range)
            mse = calculate_mse(output_slice, img_fl_slice, mask_fl_slice)
            test_state_item.add("ssim", ssim)
            test_state_item.add("mse", mse)

            psnr = 10 * torch.log10((torch.max(img_fl_slice) ** 2) / (mse.mean() + 1e-12)).view(-1, 1, 1, 1)
    ...
    logger.info(
        f"psnr: {test_state_item.mean('psnr').item():.4f}, ssim: {test_state_item.mean('ssim').item():.4f}, mse: {test_state_item.mean('mse').item():.6f}"
    )

    if not save_val:
        return

    flair_raw_img = _data[DataKey.FL_Raw_Img]
    flair_raw_sen = _data[DataKey.FL_Raw_Sen]
    fl_biasfield = _data[DataKey.FL_BIASFIELD]

    train_div = "results"
    os.makedirs(test_dir / train_div, exist_ok=True)
    save_dict = {
        "fl": img_fl.cpu().numpy()[0, ...],
        "out": output.cpu().numpy()[0, ...],
        "fl_mask": mask_fl.cpu().numpy()[0, ...],
        "t1": img_t1.cpu().numpy()[0, ...],
        "t2": img_t2.cpu().numpy()[0, ...],
        "fl_raw_img": flair_raw_img.cpu().numpy()[0, ...],
        "fl_raw_sen": flair_raw_sen.cpu().numpy()[0, ...],
        "fl_biasfield": fl_biasfield.cpu().numpy()[0, ...],
