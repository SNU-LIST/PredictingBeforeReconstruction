...
from params import ModelConfig, config
...
NET_LONGITUDINAL = PredPriorRecon | torch.nn.DataParallel[PredPriorRecon]
...
def test_part_predrecon(
    ...
    data_range = torch.amax(target_abs.view(target.shape[0], -1), dim=1)
    ...
    for slice in range(output.shape[1]):
        ...
        if torch.any(mask_slice):
            ssim = calculate_ssim(output_slice, target_slice, mask_slice, data_range)
            mse = calculate_mse(output_slice, target_slice, mask_slice)
            test_state_item.add("ssim", ssim)
            test_state_item.add("mse", mse)

            psnr = 10 * torch.log10((torch.max(target_slice) ** 2) / (mse.mean() + 1e-12)).view(-1, 1, 1, 1)
    ...
    # logger.info(
    #     f"psnr: {test_state_item.mean('psnr').item():.4f}, ssim: {test_state_item.mean('ssim').item():.4f}, psnr tot: {test_state.mean('psnr').item():.4f}, ssim tot: {test_state.mean('ssim').item():.4f}"  # noqa: E501
    # )
    ...
    save_dict = {
        ...
        "mask": target_mask.cpu().detach().numpy()[0, ...],
    }
...
def test_part(
    ...
    for _data in valid_loader:
        if ModelType(config.model_type) == ModelType.PredRecon:
            test_part_predrecon(
                _data=_data,
...
class Tester:
    ...
    def _set_network(
        ...
        config.prior_key = self.config.prior_key
        if "config" in longitudinal_checkpoint_data:
            config.prior_key = longitudinal_checkpoint_data["config"]["prior_key"]
            self.config.prior_key = longitudinal_checkpoint_data["config"]["prior_key"]
            self.config.target_key = longitudinal_checkpoint_data["config"]["target_key"]
            self.config.target_mask_key = longitudinal_checkpoint_data["config"]["target_mask_key"]
            train_config = longitudinal_checkpoint_data["config"]
            logger.info(separator())
            logger.info("Train Config from Checkpoint")
            for k in train_config:
                logger.info(f"{k}:{train_config[k]}")

        self.network_longitudinal = PredPriorRecon(device=self.config.device, modelconfig=self.modelconfig)
        ...
        try:
            ...
            logger.error("Error loading state dict: please check the checkpoint and model parameters")
            raise
