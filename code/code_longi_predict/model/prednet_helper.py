...
def train_part(
    ...
    validate_tensors([img, img_prev, meta])
    validate_tensor_dimensions([img, img_prev], 4)

    time = torch.randint(low=0, high=num_timesteps, size=(img.shape[0], 1), device=device)
...
def validate_part(
    ...
    validate_tensors([img_prev, meta])
    validate_tensor_dimensions([img_prev], 4)

    B, C, W, H = img_prev.shape
