...
def test_part_predrecon(
    ...
    logger.info(f"psnr: {test_state.mean('psnr').item():.4f}, ssim: {test_state.mean('ssim').item():.4f}, mse: {test_state.mean('mse').item():.6f}")
...
class Tester:
    ...
    def _set_network(
        ...
        if "config" in longitudinal_checkpoint_data:
            ...
            self.config.prior_key = config.prior_key
