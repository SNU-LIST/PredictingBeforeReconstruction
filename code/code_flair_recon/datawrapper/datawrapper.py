...
from datawrapper.augment import Augment
...
class DataWrapper(Dataset):
    ...
    def __getitem__(
        ...
        target = torch.from_numpy(file_mat[self.target_key]).type(torch.float)
        target = target.type(torch.complex64)
        ...
        for pk in self.prior_keys:
            ...
            # _prior = _prior.type(torch.complex64)
        ...
        if self.training_mode:
            prior, target = self._augment(prior, target)
